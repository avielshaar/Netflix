...
export const ContentProvider = ({ children }) => {
  ...
  const addItemToMyList = async (content) => {
    if (!myList.some((item) => item._id === content._id)) {
      try {
        const response = await axios.post(`/api/v1/users/addtolist/${content._id}`, userInfo, {
          headers: { authorization: `Bearer ${userInfo.token}` },
        });
  ...
  const removeItemFromMyList = async (content) => {
    ...
    try {
      const response = await axios.post(`/api/v1/users/removefromlist/${content._id}`, userInfo, {
        headers: { authorization: `Bearer ${userInfo.token}` },
      });
      setMyList((prevList) => prevList.filter((item) => item._id !== content._id));
    } catch (error) {
